...
- **Deployed** 2026-07-11: live at https://slowdoctor-type.leadprs-seoul.workers.dev (account leadprs.seoul@gmail.com). Remote D1 `slowdoctor-type` created (id in `wrangler.jsonc`), all 4 migrations applied, `classic` 39 + `daily` 23 seeded. `news`/`medical` fill via the daily cron (21:00 UTC = 06:00 KST).
...
- **Custom domain type.slowdoctor.dev blocked**: the `slowdoctor.dev` zone lives on a *different* Cloudflare account than the worker (API error 10083). Fix = deploy the worker from the zone-owning account (`wrangler login` there, restore a `routes` custom_domain entry) or attach via that account's dashboard. Until then the workers.dev URL is canonical.
