...
- **Not deployed (again)** — the 2026-07-11 first deploy went to leadprs.seoul@gmail.com's Cloudflare account (the machine's wrangler session), but the `slowdoctor.dev` zone lives on a *different* account, so type.slowdoctor.dev could not attach (API error 10083). Director ordered that deployment deleted the same day (worker + D1 removed; verified 404). E2E was proven live before deletion: migrations, classic/daily serving, result-range rejection, run-verification screenshots.
- **Redeploy runbook**: `wrangler login` with the account that owns the slowdoctor.dev zone (human step — browser auth), then README "Deploy (first time)" + restore the `routes` custom_domain entry in `wrangler.jsonc` (see comment there). Do not deploy from any other account.
- **FEED_TOKEN secret intentionally not set** (Director 2026-07-11: wait for the cron instead of seeding manually). `POST /api/feed` errors until `wrangler secret put FEED_TOKEN` is run; the cron feeder does not need it.
