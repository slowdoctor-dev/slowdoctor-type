...
- **Latest deploy: 2026-07-12, manual (Deploy button), commit `e799978`** — everything from the 2026-07-12 marathon is live: migrations 0005–0012 auto-applied (track rework, accounts, feed_log, pmc rename, vocab+code seeds, fk_grade, user_history), all six tracks, difficulty filter, rankings, history sync, slowdoctor.dev theme. The first cron run under the new schema (able to fill `federal`/`pmc` and backfill fk_grade) is **2026-07-13 06:00 KST** — after it, live-verify the federal contract via `/api/health` (`federal > 0`, `last_feed.errors` empty).
